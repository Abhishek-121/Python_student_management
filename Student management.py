...
def add_student():
            ...
            if ret==0:
                ...
                cur.execute("insert into STUDENTS values(?,?,?,?,?,?)",(student_name.get(),roll_no.get(),branch.get(),phone.get(),father.get(),address.get()))
                conn.commit()
                conn.close()
...
def delete_student():
    ...
    if ret==0:
        conn=connection()
        cur=conn.cursor()
        cur.execute("DELETE FROM STUDENTS WHERE ROLL_NO=?",(roll_no.get(),))
        conn.commit()
        conn.close()
...
def update_student():
    ...
    if ret==0:
        conn=connection()
        cur=conn.cursor()
        cur.execute("UPDATE STUDENTS SET NAME=?,ROLL_NO=?,BRANCH=?,PHONE_NO=?,FATHER=?,ADDRESS=? where ROLL_NO=?",(student_name.get(),roll_no.get(),branch.get(),phone.get(),father.get(),address.get(),roll_no.get()))
        conn.commit()
        conn.close()
...
if __name__=="__main__":
    ...
    label5=Label(root,text="Father Nmae:")
